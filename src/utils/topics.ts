 ...
 */
export function validateTopicsParameters(
 ...
 * @param {AxiosResponse<Topics>} initialResponse - The `initialResponse` parameter is an object that represents the initial response.
 ...
 * @returns an object that represents the topics.
 */
export async function handleMultipleTopicsPages(
  startPage: number,
  endPage: number,
  url: string,
  initialResponse: AxiosResponse<Topics>,
  toJson?: string,
  toCsv?: string,
) {
  const topics = initialResponse.data;
  let cursor = topics.meta.next_cursor;
  url = url.split('&cursor')[0];
  for (let i = startPage + 1; i <= endPage; i++) {
    const response: AxiosResponse<Topics> = await GET(
      `${url}&cursor=${cursor}`,
    );
    if (response.status === 200) {
      topics.results = topics.results.concat(response.data.results);
      cursor = response.data.meta.next_cursor;
    } else throw new Error(`Error ${response.status}: ${response.statusText}`);
    if (i === endPage) {
      topics.meta.next_cursor = cursor;
      topics.meta.page = endPage;
    }
  }
  if (toJson)
    fs.writeFileSync(`${toJson}.json`, JSON.stringify(topics, null, 2));
  if (toCsv) convertToCSV(topics.results, toCsv);
 ...
 * @returns an object that represents the topics.
 */
export async function handleAllTopicsPages(
  url: string,
  initialResponse: AxiosResponse<Topics>,
  toJson?: string,
  toCsv?: string,
  ...
  const topics = initialResponse.data;
  console.log('length', topics.results.length);
  ...
  for (let i = 2; i <= totalPages; i++) {
    const response: AxiosResponse<Topics> = await GET(`${url}${cursor}`);
    ...
    if (i === totalPages) {
      topics.meta.next_cursor = cursor;
      topics.meta.page = totalPages;
    }
  }
  if (toJson)
    fs.writeFileSync(`${toJson}.json`, JSON.stringify(topics, null, 2));
  if (toCsv) {
    convertToCSV(topics.results, toCsv);
  }
  return topics;
}
