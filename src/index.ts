...
class OpenAlex {
  ...
  //   const { retriveAllPages, searchField, search, fileName, toJson, startPage, endPage } = searchParameters;
  //   if (retriveAllPages && (startPage || endPage)) throw new Error('startPage and endPage are not allowed with retriveAllPages');
  //   if (toJson && !fileName) throw new Error('fileName is required when toJson is true');
  ...
  //     }
  //     if (toJson) fs.writeFileSync(`${fileName}.json`, JSON.stringify(works, null, 2));

  //     return works;
  //   }
  //   if (retriveAllPages) {
  //     const totalPages = calculatePages(200, response.data.meta.count);
  //     const works = response.data;
  //     // remove page from url without split

  //     console.log('total number of pages ', totalPages);
  //     console.log('page', 1, 'response', response.status);
  //     for (let i = 2; i <= totalPages; i++) {
  //       const response: AxiosResponse<Works> = await GET(`${url}&page=${i}`);
  //       console.log('page', i, 'response', response.status);
  //       if (response.status === 200) works.results = works.results.concat(response.data.results);
  //       else throw new Error(`Error ${response.status}: ${response.statusText}`);
  //       if (i === totalPages) works.meta.page = totalPages; // save it to a file
  //     }
  //     if (toJson) fs.writeFileSync(`${fileName}.json`, JSON.stringify(works, null, 2));
